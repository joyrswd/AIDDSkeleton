...
- When a verification claim or later project decision needs retained evidence, preserve only the material needed for that continuing purpose. Retained workbench evidence should identify the actual target or version using an unambiguous identity appropriate to the environment, relevant conditions, verification method, actual result, directly verified scope, and material unverified scope. Git commits, branch names, CI run identifiers, or repository files are possible identities or storage mechanisms, not required ones.
...
- When a status, traceability, or index entry references retained evidence or reference material to support current coverage, the referenced material must be applicable to the asserted state. Do not leave a superseded result as the sole support after later applicable verification exists; if earlier evidence remains applicable, state the basis for that applicability rather than implying that it exercised a later target.
...
- Keep current effective relationships in status and traceability documents. Do not append historical runs there and do not require a project-managed historical evidence archive. Dispose of, supersede, or promote workbench verification material according to its continuing value.
