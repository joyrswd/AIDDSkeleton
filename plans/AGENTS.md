...
- Workbench retention may bridge active work and handoff, but do not use a closed workbench unit indefinitely as the sole project-managed basis for a long-lived current verified claim. When that need becomes continuing rather than transitional, use a suitable durable native or external basis, promote only the minimum project-managed material needed to reassess the claim to `references/`, or downgrade the claim. One proportional basis may support multiple related claims; do not create per-claim or per-run repository files merely to satisfy this lifecycle rule.
