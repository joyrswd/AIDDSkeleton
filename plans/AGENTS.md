...
- Treat verification-basis retention in `workbench/` as transitional only while the material still serves active work, an immediate handoff, or unresolved reconciliation belonging to the work unit. Closing the unit does not by itself require promotion. If a current verified claim must remain supportable after those work-unit responsibilities are complete and the workbench material would otherwise remain its sole project-managed basis, that basis has a durable responsibility: use a suitable durable native or external basis, promote only the minimum project-managed material needed to reassess the claim to `references/`, or downgrade the claim. One proportional basis may support multiple related claims; do not create per-claim or per-run repository files merely to satisfy this lifecycle rule.
