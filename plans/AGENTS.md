...
- Except for an approved dedicated non-normative evidence location governed below, keep system documents directly under `plans/system/`; use document boundaries rather than subdirectories to separate their responsibilities.
...
- Keep `requirements/`, `design/`, and `testing/` as normative category boundaries. Do not treat execution-result or evidence records—such as CI run reports, command output, measured results, artifact identities, or completed verification logs—as category documents merely because they were produced while verifying the corresponding requirements. When a document mixes normative verification criteria with execution results, classify the materially constraining statements before separation so the verification intent remains in normative testing.
- A testing index may link to current applicable execution evidence needed to judge current coverage, but the evidence record itself is not a testing source of truth and must not be listed or described as a normative testing specification or used as a general execution-history catalog.
...
- Record execution-specific evidence outside the normative `requirements/`, `design/`, and `testing/` category directories in a project-defined approved non-normative evidence location. Every evidence location must remain anchored to an existing responsibility: application-specific evidence belongs within the responsible `apps/<app>/` scope, and system-wide or cross-application evidence belongs within the `system/` responsibility; evidence must not introduce a peer classification alongside `plans/apps/` and `plans/system/`. The canonical evidence location or routing for each responsibility must be discoverable from its existing navigation entry point—`<app>_index.md` for an application and `system_index.md` for system-wide or cross-application evidence—without requiring those indexes to enumerate every evidence record. A project may define or later adopt a dedicated non-normative evidence location within that owning responsibility when evidence volume, retention, navigation, or another durable responsibility warrants it. If no such location is defined, use the responsible `apps/<app>/` area for application-specific evidence and `system/` for system-wide or cross-application evidence as the default. A dedicated evidence location does not create a new normative documentation category; creating a new directory under `plans/` remains subject to the structural approval rule below. Evidence or result records own the immutable execution identity—the exact commit SHA or equivalent immutable version when available—plus environment, commands, run identifiers, artifacts, actual results, directly verified scope, and material unverified scope; status, traceability, and testing documents should link to those records rather than duplicate the details. A moving branch name such as `main` is context, not a substitute for immutable execution identity; if the exact executed identity cannot be established, record it as unknown rather than infer it.
...
- When that migration finds execution-result or evidence content inside `requirements/`, `design/`, or `testing/`, classify mixed documents at statement or section level before relocation. Keep verification thresholds, required observations, sufficient-evidence criteria, and other adopted verification intent in normative testing; move only evidence-owned execution history such as actual measured values, run or commit identities, artifacts, command output, and PASS/FAIL history to the approved non-normative evidence location. Reconcile inbound links for relocated evidence, preserve evidence content and history that still has evidential value, and do not expand the migration into unrelated repository-wide history cleanup.
