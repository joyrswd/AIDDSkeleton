...
- When governance retires a project-definition artifact or classification, treat existing instances as migration cases: preserve only still-authoritative semantics in responsible SoTs, reconcile affected index routing/current state/VB and inbound links, then remove retired artifact/stale references in the same coherent migration; do not preserve retired structure/non-required correspondence merely for legacy compatibility.
