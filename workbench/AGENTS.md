...
- Treat verification-basis retention in the workbench as transitional while the material still serves active work, an immediate handoff, or unresolved reconciliation belonging to the work unit. Closing a unit does not by itself require promotion. If a current verified claim must remain supportable after those work-unit responsibilities are complete and the workbench material would otherwise remain its sole project-managed basis, follow the durable-basis lifecycle in `plans/AGENTS.md`: use a suitable durable native or external basis, promote only the minimum project-managed material needed to reassess the claim to `references/`, or downgrade the claim.
