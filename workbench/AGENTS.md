...
- When testing a candidate shared-governance baseline against an initialized consumer, use the candidate governance together with the consumer's pre-existing protected local rules as the governing input. A prompt may identify questions, risks, or areas to falsify, but must not supply expected classifications, a target diff, or repair instructions as authority for the result.
- Treat such consumer regression as evaluation rather than adoption. Keep the candidate application isolated and reversible until the candidate is accepted for that consumer; completing the regression does not by itself authorize replacing the consumer's adopted governance or publishing the consumer changes. No particular version-control mechanism is required.
