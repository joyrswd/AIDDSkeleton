...
- Use an identity appropriate to the source or environment. A commit SHA, release identifier, artifact digest, dataset version, external snapshot date, run identifier, or other unambiguous identity may be useful, but no particular version-control or execution system is required.
...
- Project-managed reference material may be revised or superseded when its responsibility requires it, but preserve enough provenance to distinguish an updated reference from the observation or evidence from which it was derived.
...
- Before moving or deleting material, inspect documentation, workbench units, implementations, active work, provenance records, licensing obligations, usage references, and any continuing evidential or maintenance need.
