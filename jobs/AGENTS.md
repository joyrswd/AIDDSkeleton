...
- Updating a retained job means reconciling working material or control state for its existing purpose and basis. Permission to update that job does not by itself authorize a new purpose, material scope/AC/RB/design change, priority change, or current-work commitment.
- Refine, correct, reorder, or re-split an existing job when the resulting work remains required by its existing approved basis/scope/AC or root Permission / Scope otherwise permits that work in the current task. Do not absorb an independently completable improvement, future opportunity, or adjacent follow-up merely because it was discovered while updating the job.
- When the original job can still satisfy its pre-existing approved basis without a discovered item, treat that item as a separate assessment input unless current authority independently makes it part of the job. Apply root Assessment and Feedback and retain a separate follow-up only when continuing value justifies retention.
- Do not create or enlarge retained job content merely to record every discovered suggestion, observation, or possible improvement. Discovery and apparent validity do not make an item part of the existing job or require repository retention.
- The actor updating a job is responsible for re-evaluating whether its top-level lifecycle marker still matches the resulting current state and reconciling the marker when the applicable lifecycle rules require a transition. This is maintenance of represented state within existing authority, not independent authority to activate work or keep work active.
- Content mutation, recency, metadata/evidence refresh, or retention of new context does not by itself activate, deactivate, or reopen a job. `_` → `+` still requires current approved basis and applicable authority for active lifecycle work; `+` remains valid only while active lifecycle work or required transition obligations remain. If an update leaves only justified inactive value, reconcile under the existing `+` → `_`/retention rules instead of preserving `+` because the job was recently changed.
