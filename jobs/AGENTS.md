...
- After `jobs.blocked`, a Consumer Hook may continue only by applying ordinary lifecycle discovery and `_` → `+` selection rules to another independently executable job within existing authority. It must not treat the blocking occurrence as basis for new work or bypass approved basis, dependency, entry condition, inactive-parent discovery gating, or user-owned scope, priority, RB, material design, or acceptance decisions. If the blocking condition itself requires a user-owned decision, or if another job cannot be selected without a material priority choice or other such decision, do not automatically select one; surface the applicable decision/blocker instead. A valid successor activation reaches `jobs.activated` normally.
