...
- Consumer Hooks are consumer-local instructions, not SoTs or generally inherited rules. Read/apply only when the Hook is reached, except explicit inspection/maintenance. They remain subordinate to root, applicable routes, SoTs, Permission / Scope, Safety, and required lifecycle/transfer behavior.
